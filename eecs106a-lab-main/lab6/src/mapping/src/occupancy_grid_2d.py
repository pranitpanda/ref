...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            voxels = set()
            for i in np.arange(0,r,min(self._x_res, self._y_res)/2):
                x = (sensor_x + (r-i)*np.math.cos(angle))
                y = (sensor_y + (r-i)*np.math.sin(angle))
                voxel = self.PointToVoxel(x,y)
                if voxel not in voxels:
                    if i == 0 :
                        # do occupied update
                        self._map[voxel] += self._occupied_update
                        self._map[voxel] = min(self._map[voxel], self._occupied_threshold)
                    else:
                        # do the free update
                        self._map[voxel] += self._free_update
                        self._map[voxel] = max(self._map[voxel], self._free_threshold)
                    voxels.add(voxel)
