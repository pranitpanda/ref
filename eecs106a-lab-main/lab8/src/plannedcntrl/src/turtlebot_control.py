...
def controller(waypoint):
  ...
  while not rospy.is_shutdown():
    try:
      trans_odom_to_base_link = tfBuffer.lookup_transform("base_footprint", "odom", rospy.Time())
      ...
      integ += np.dot(dt, error)## TODO: integral is summing up error over time, so what would we expect to add on to our integral term tracker here?
...
def planning_callback(msg):
  try:
    ...
    for waypoint in trajectory:
      controller(waypoint) ## TODO: write a loop to loop over our waypoints and call the controller function on each waypoint
...
if __name__ == '__main__':
  ...
  rospy.Subscriber("/goal_point", Point, callback=planning_callback) ## TODO: what are we subscribing to here?
