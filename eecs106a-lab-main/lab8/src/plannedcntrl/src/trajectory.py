...
def bezier_curve(p0, p1, p2, p3, t):
    ...
    return p0*(1-t)**3 + 3*t*p1*(1-t)**2 + 3*(t**2)*p2*(1-t) + (t**3)*p3
...
def plan_curved_trajectory(target_position):
    ...
    Parameters:
    - target_position: A tuple (x, y) representing in the world frame
    ...
    x2 =  np.cos(yaw)* target_position[0] - np.sin(yaw)*target_position[1] + x1 
    y2 =  np.sin(yaw)* target_position[0]+ np.cos(yaw)*target_position[1] + y1
...
if __name__ == '__main__':
    ...
    trajectory = plan_curved_trajectory((0.3,0.5))
